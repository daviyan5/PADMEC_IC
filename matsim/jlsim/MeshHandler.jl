...
function __init__()
        ...
        def convert_to_pymat(mesh_filename, **kwargs):
            import scipy.io as sio
            sio.savemat(mesh_filename[:-4] + ".mat", kwargs)
            print("")
            import os
            os.remove(mesh_filename[:-4] + ".msh")
        """
...
function convert_to_pymat(mesh :: Mesh)
    py"convert_to_pymat"(   mesh.mesh_filename, 
                            nvols = mesh.nvols, nfaces = mesh.nfaces,
                            volumes = mesh.volumes, faces_areas = mesh.faces_areas,
                            volumes_centers = mesh.volumes_centers, faces_centers = mesh.faces_centers, nodes_centers = mesh.nodes_centers,
                            internal_faces = mesh.internal_faces, boundary_faces = mesh.boundary_faces,
                            volumes_adj = mesh.volumes_adj, internal_faces_adj = mesh.internal_faces_adj, boundary_faces_adj = mesh.boundary_faces_adj,
                            vols_connectivity = mesh.vols_connectivity, faces_connectivity = mesh.faces_connectivity)
end
...
for mesh_file in readdir("./mesh")
    if occursin(".msh", mesh_file)
        mesh = MeshHandler.Mesh("./mesh/$mesh_file")
        show(mesh)
        MeshHandler.convert_to_pymat(mesh)
    end
end
