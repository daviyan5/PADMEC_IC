...
def FiniteDiff(f, n1, n2, l1 = 0, l2 = 1, l3 = 0, l4 = 1, K = 0):
    ...
    A = np.diag(v = 2 * (1 + ratio) * np.ones(n1 * n2), k = 0) + np.diag(v = -np.ones(n1 * n2 - n2), k = n2) + np.diag(v = -np.ones(n1 * n2 - n2), k = -n2) + np.diag(v = -ratio * np.ones(n1 * n2 - 1), k = 1) + np.diag(v = -ratio * np.ones(n1 * n2 - 1), k = -1)       # u[k+1]
    print("\n############### A ###############\n\n",A,"\n\n#################################\n")
...
def main():
    f = lambda x1, x2: np.exp(x1+1) + np.exp(x2+1)
    x1, x2, U = FiniteDiff(f, 100, 100, K = 0)
