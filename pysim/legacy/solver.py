...
class Solver:
    ...
    def _set_boundary_conditions(self, bc, f):
        ...
        if bc == "dirichlet":
            ...
            d_values = (f(x, y, z)) #+ f(xi, yi, zi) + f(xj, yj, zj) + f(xk, yk, zk) + f(xl, yl, zl) +
                        #f((x + xi) / 2, (y + yi)/2, (z + zi)/2) + f((x + xj) / 2, (y + yj)/2, (z + zj)/2) + 
                        #f((x + xk) / 2, (y + yk)/2, (z + zk)/2) + f((x + xl) / 2, (y + yl)/2, (z + zl)/2)) / 9
            ...
            #self.A_d[d_volumes] = 1.
            self.A_d = csr_matrix((self.A_d, (row_index, col_index)), 
                                   shape=(self.nvols, self.nvols))
            
            #coords = self.mesh.volumes.center[d_volumes]
            #xv, yv, zv = coords[:, 0], coords[:, 1], coords[:, 2]
            #self.b_d = np.zeros(self.nvols + len(d_nodes))
            #self.b_d[d_nodes] = d_values
